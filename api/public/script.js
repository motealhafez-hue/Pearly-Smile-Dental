...
const HomeBlogTeaser = (function () {
  ...
  function heroFocalStyleAttr(p) {
    if (!p || typeof p !== "object") return "";
    let x = Number(p.hero_focal_x);
    let y = Number(p.hero_focal_y);
...
class FAQAccordion {
  ...
  static attachListener(item) {
    ...
    if (answer && !item.classList.contains("active")) answer.style.maxHeight = "0px";
...
/** One primary nav item matches the current page (data-page + hash on home). Drives desktop underline via CSS. */
function syncPrimaryNavAriaCurrent() {
  const ul = document.querySelector("header.mobile-nav #mobileNav ul");
  if (!ul) return;
  ...
  if (page === "home") {
    if (hash === "stats") link = ul.querySelector('a[href="#stats"]');
    else if (hash === "services") link = ul.querySelector('a[href="#services"]');
    else if (hash === "booking") link = ul.querySelector('a[href="#booking"]');
    else link = ul.querySelector('a[href="#home"]');
  } else if (page === "about") {
    link = ul.querySelector('a[href="about.html"]');
  } else if (page === "team") {
    link = ul.querySelector('a[href="team.html"]');
  } else if (page === "services" || page.startsWith("service_")) {
    link = ul.querySelector('a[href="services.html"]');
  } else if (page === "offers") {
    link = ul.querySelector('a[href="offers.html"]');
  } else if (page === "blog" || page === "blog_post") {
    link = ul.querySelector('a[href="blog.html"]');
  }
