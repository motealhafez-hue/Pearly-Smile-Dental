...
// hero_image, hero_focal_x/y, meta_title, meta_description, content_html, related_slugs, published.
(function () {
  "use strict";
  ...
  /** hero_focal_x / hero_focal_y from API (0–100); default center. */
  function heroFocalCss(post) {
    if (!post || typeof post !== "object") return "object-position: 50% 50%";
    var x = Number(post.hero_focal_x);
    var y = Number(post.hero_focal_y);
    if (!Number.isFinite(x)) x = 50;
    if (!Number.isFinite(y)) y = 50;
    x = Math.min(100, Math.max(0, x));
    y = Math.min(100, Math.max(0, y));
    var s = Number(post.hero_scale != null ? post.hero_scale : post.hero_image_scale);
    if (!Number.isFinite(s)) s = 1;
    s = Math.min(3, Math.max(0.5, s));
    var zoom = Math.abs(s - 1) > 0.001 ? "; transform: scale(" + s + "); transform-origin: " + x + "% " + y + "%" : "";
    return "object-position: " + x + "% " + y + "%" + zoom;
  }

  function heroFocalStyleAttr(post) {
    return ' style="' + heroFocalCss(post) + '"';
  }
    ...
    try {
      ...
      if (featuredFig && featuredImg) {
        if (heroUrl) {
          ...
          featuredImg.setAttribute("style", heroFocalCss(post));
          ...
          featuredImg.removeAttribute("style");
        }
      }
