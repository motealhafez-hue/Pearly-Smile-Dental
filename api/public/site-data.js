...
(function (global) {
  ...
  /**
   * CMS image focal (0–100).
   * New admin stores per-image-field values as: `${key}_focal_x` / `${key}_focal_y`.
   * Legacy shapes may still use: `image_focal_x/y` or `hero_focal_x/y`.
   */
  function focalPairPct(row, key) {
    if (!row || typeof row !== "object") return { x: 50, y: 50 };
    var k = key != null ? String(key).trim() : "";
    var x =
      k && row[k + "_focal_x"] != null
        ? Number(row[k + "_focal_x"])
        : row.image_focal_x != null
        ? Number(row.image_focal_x)
        : Number(row.hero_focal_x);
    var y =
      k && row[k + "_focal_y"] != null
        ? Number(row[k + "_focal_y"])
        : row.image_focal_y != null
        ? Number(row.image_focal_y)
        : Number(row.hero_focal_y);
  ...
  function objectPositionInline(row, key) {
    var f = focalPairPct(row, key);
    return "object-position: " + f.x + "% " + f.y + "%";
  }

  function scaleVal(row, key) {
    if (!row || typeof row !== "object") return 1;
    var k = key != null ? String(key).trim() : "";
    var s = k && row[k + "_scale"] != null ? Number(row[k + "_scale"]) : Number(row.image_scale);
    if (!Number.isFinite(s)) s = 1;
    return Math.min(3, Math.max(0.5, s));
  }

  function objectTransformInline(row, key) {
    var f = focalPairPct(row, key);
    var s = scaleVal(row, key);
    if (!s || Math.abs(s - 1) < 0.001) return "";
    return "transform: scale(" + s + "); transform-origin: " + f.x + "% " + f.y + "%";
  }
  ...
  function renderHomeServices(container, data, lang) {
    ...
    container.innerHTML = services
      .map(function (svc) {
        ...
        var foc = objectPositionInline(svc, "image");
        var zoom = objectTransformInline(svc, "image");
        return (
          ...
          '" alt="' +
          alt +
          '" loading="lazy" decoding="async" width="640" height="400" fetchpriority="low" style="' +
          escapeHtml(foc + (zoom ? "; " + zoom : "")) +
          '" />' +
          "</div>" +
  ...
  function serviceFlipCard(svc, _delay, lang) {
    ...
    const imgHtml = imgUrl
      ? '<div class="service-photo-wrap"><img class="service-photo" loading="lazy" decoding="async" alt="" src="' +
        ...
        '" style="' +
        escapeHtml(
          objectPositionInline(svc, "image") +
            (objectTransformInline(svc, "image") ? "; " + objectTransformInline(svc, "image") : "")
        ) +
        '" /></div>'
      : "";
    const href = svc.href || "#";
  ...
  function renderHomeAbout(container, data, lang) {
    ...
    var vis = ha.visual && typeof ha.visual === "object" ? ha.visual : null;
    var imgBlock = imgUrl
      ? '<img class="about-showcase__visual-photo" src="' +
        escapeHtml(imgUrl) +
        '" alt="' +
        escapeHtml(chip || "") +
        '" loading="lazy" decoding="async" data-cms-bound="1" style="' +
        escapeHtml(
          objectPositionInline(vis, "image") +
            (objectTransformInline(vis, "image") ? "; " + objectTransformInline(vis, "image") : "")
        ) +
        '" />'
      : "";
    ...
    doctors.forEach((doc, i) => {
      ...
      if (imgHost) {
        ...
        if (url) {
          ...
          const fp = focalPairPct(doc, "image");
          const sc = scaleVal(doc, "image");
          img.src = url;
          img.alt = "";
          img.loading = "lazy";
          img.decoding = "async";
          ...
          if (Math.abs(sc - 1) > 0.001) {
            img.style.transformOrigin = fp.x + "% " + fp.y + "%";
  ...
  function teamDoctorArticle(doc, index, lang) {
    ...
    const sc = scaleVal(doc, "image");
    const imgHtml = imgUrl
      ? '<img class="doc-photo" loading="lazy" decoding="async" alt="" src="' +
        escapeHtml(imgUrl) +
        ...
        fp.y +
        "%;" +
        (Math.abs(sc - 1) > 0.001 ? " transform: scale(" + sc + "); transform-origin: " + fp.x + "% " + fp.y + "%;" : "") +
        '" />'
      : "";
    return (
  ...
  function renderOffersGrid(container, data, lang) {
    ...
    offers.forEach((off, i) => {
      ...
      if (img && off && off.image) {
        ...
        if (url) {
          img.style.backgroundImage = "url('" + url.replace(/'/g, "%27") + "')";
          const fp = focalPairPct(off, "image");
          img.style.backgroundPosition = fp.x + "% " + fp.y + "%";
          const sc = scaleVal(off, "image");
          if (Math.abs(sc - 1) > 0.001) {
            const pct = Math.round(sc * 1000) / 10;
            img.style.backgroundSize = pct + "% " + pct + "%";
          } else {
  ...
  function renderCareersJobs(container, data, lang) {
    if (!container) return;
    const list = (data && data.careersJobs) || [];
    if (!Array.isArray(list) || list.length === 0) {
      container.innerHTML =
        '<div class="empty-hint" style="padding:1rem;opacity:.7">' +
        escapeHtml(lang === "en" ? "No open positions at the moment." : "لا توجد وظائف متاحة حالياً.") +
        "</div>";
      return;
    }
    container.innerHTML = list
      .map(function (j) {
        ...
        const title = j.title ? t(j.title, lang) : "";
        const dept = j.department ? t(j.department, lang) : "";
        const emp = j.employment_type ? t(j.employment_type, lang) : "";
        const loc = j.location ? t(j.location, lang) : "";
        const short = j.short ? t(j.short, lang) : "";
        const id = j.id != null ? String(j.id).trim() : "";
        const apply = j.apply_url != null ? String(j.apply_url).trim() : "";
        const detailHref = "career-job.html?id=" + encodeURIComponent(id || "");
        const btn =
          '<a class="btn btn-primary" href="' +
          escapeHtml(detailHref) +
          '">' +
          escapeHtml(lang === "en" ? "Apply now" : "قدّم الآن") +
          "</a>";
        return (
          '<article class="job-card" data-cms-bound="1">' +
          '<div class="job-card__head">' +
          '<h3 class="job-card__title">' +
          escapeHtml(title) +
          "</h3>" +
          '<div class="job-card__meta">' +
          (dept ? '<span class="pill">' + escapeHtml(dept) + "</span>" : "") +
          (emp ? '<span class="pill">' + escapeHtml(emp) + "</span>" : "") +
          (loc ? '<span class="pill">' + escapeHtml(loc) + "</span>" : "") +
          "</div></div>" +
          (short ? '<p class="job-card__desc">' + escapeHtml(short) + "</p>" : "") +
  ...
  function renderInsuranceProviders(container, data, lang) {
    if (!container) return;
    const list = (data && data.insuranceProviders) || [];
    if (!Array.isArray(list) || list.length === 0) {
      container.innerHTML =
        '<div class="empty-hint" style="padding:1rem;opacity:.7">' +
        escapeHtml(lang === "en" ? "Insurance providers will be listed here." : "سيتم عرض مزودي التأمين هنا.") +
        "</div>";
      return;
    }
    container.innerHTML = list
      .map(function (p) {
        ...
        const name = p.name ? t(p.name, lang) : "";
        const short = p.short ? t(p.short, lang) : "";
        const cov = p.coverage ? t(p.coverage, lang) : "";
        const logo = p.logo != null ? String(p.logo).trim() : "";
        const logoHtml = logo
          ? '<div class="ins-card__logo"><img src="' +
            escapeHtml(absMediaUrl(logo)) +
            '" alt="' +
            escapeHtml(name) +
            '" loading="lazy" decoding="async" /></div>'
          : '<div class="ins-card__logo ins-card__logo--empty" aria-hidden="true">+</div>';
        const id = p.id != null ? String(p.id).trim() : "";
        const hrefRaw = p.href != null ? String(p.href).trim() : "";
        const href = hrefRaw || ("insurance-provider.html?id=" + encodeURIComponent(id || ""));
        return (
          '<a class="ins-card" data-cms-bound="1" href="' +
          escapeHtml(href) +
          '">' +
          '<div class="ins-card__head">' +
          logoHtml +
          '<div class="ins-card__name">' +
          escapeHtml(name) +
          "</div>" +
          "</div>" +
          (short ? '<p class="ins-card__desc">' + escapeHtml(short) + "</p>" : "") +
          (cov ? '<div class="ins-card__cov">' + escapeHtml(cov) + "</div>" : "") +
          "</a>"
        );
      })
      .filter(Boolean)
  ...
  function renderCareerJobDetail(container, data, lang) {
    if (!container) return;
    try {
      const params = new URLSearchParams(String(global.location && global.location.search ? global.location.search : ""));
      const id = String(params.get("id") || "").trim();
      const list = (data && data.careersJobs) || [];
      const job = Array.isArray(list) ? list.find((x) => x && String(x.id || "") === id) : null;
      const titleEl = document.getElementById("jobTitle");
      const shortEl = document.getElementById("jobShort");
      const metaEl = document.getElementById("jobMeta");
      const aboutTitle = document.getElementById("jobAboutTitle");
      const reqTitle = document.getElementById("jobReqTitle");
      const benTitle = document.getElementById("jobBenTitle");
      ...
      const applyTop = document.getElementById("jobApplyTop");
      const applyBottom = document.getElementById("jobApplyBottom");
      if (!job) {
        if (titleEl) titleEl.textContent = lang === "en" ? "Job not found" : "الوظيفة غير موجودة";
        if (shortEl) shortEl.textContent = lang === "en" ? "Please go back to Careers page." : "ارجع إلى صفحة الوظائف.";
        return;
      }
      const title = job.title ? t(job.title, lang) : "";
      const dept = job.department ? t(job.department, lang) : "";
      const emp = job.employment_type ? t(job.employment_type, lang) : "";
      const loc = job.location ? t(job.location, lang) : "";
      const short = job.short ? t(job.short, lang) : "";
      const desc = job.description ? t(job.description, lang) : "";
      const req = job.requirements ? t(job.requirements, lang) : "";
      const ben = job.benefits ? t(job.benefits, lang) : "";
      const applyUrl = job.apply_url != null ? String(job.apply_url).trim() : "";

      if (titleEl) titleEl.textContent = title;
      if (shortEl) shortEl.textContent = short;
      if (aboutTitle) aboutTitle.textContent = lang === "en" ? "About this role" : "عن الوظيفة";
      if (reqTitle) reqTitle.textContent = lang === "en" ? "Requirements" : "المتطلبات";
      if (benTitle) benTitle.textContent = lang === "en" ? "Benefits" : "المزايا";

      if (metaEl) {
        metaEl.innerHTML =
          (dept ? '<div><strong>' + escapeHtml(lang === "en" ? "Department" : "القسم") + ":</strong> " + escapeHtml(dept) + "</div>" : "") +
          (emp ? '<div><strong>' + escapeHtml(lang === "en" ? "Type" : "النوع") + ":</strong> " + escapeHtml(emp) + "</div>" : "") +
          (loc ? '<div><strong>' + escapeHtml(lang === "en" ? "Location" : "الموقع") + ":</strong> " + escapeHtml(loc) + "</div>" : "");
      }

      if (descEl) descEl.innerHTML = desc ? "<p>" + escapeHtml(desc) + "</p>" : '<p style="opacity:.7">' + escapeHtml(lang === "en" ? "Details will be added soon." : "سيتم إضافة التفاصيل قريباً.") + "</p>";
      if (reqEl) reqEl.innerHTML = req ? "<p>" + escapeHtml(req) + "</p>" : '<p style="opacity:.7">—</p>';
      if (benEl) benEl.innerHTML = ben ? "<p>" + escapeHtml(ben) + "</p>" : '<p style="opacity:.7">—</p>';

      const btnHtml = '<a class="btn btn-primary" href="' + escapeHtml(applyUrl || "contact.html") + '"' + (applyUrl ? ' target="_blank" rel="noopener"' : "") + ">" + escapeHtml(lang === "en" ? "Apply now" : "قدّم الآن") + "</a>";
      if (applyTop) applyTop.innerHTML = btnHtml;
      if (applyBottom) applyBottom.innerHTML = btnHtml;
      markCms(container);
    } catch (e) {
      /* no-op */
    }
  ...
  function renderInsuranceProviderDetail(container, data, lang) {
    if (!container) return;
    try {
      const params = new URLSearchParams(String(global.location && global.location.search ? global.location.search : ""));
      const id = String(params.get("id") || "").trim();
      const list = (data && data.insuranceProviders) || [];
      const prov = Array.isArray(list) ? list.find((x) => x && String(x.id || "") === id) : null;
      const nameEl = document.getElementById("insName");
      const shortEl = document.getElementById("insShort");
      const logoEl = document.getElementById("insLogo");
      const covTitle = document.getElementById("insCovTitle");
      const covEl = document.getElementById("insCoverage");
      const srvEl = document.getElementById("insServices");
      if (!prov) {
        if (nameEl) nameEl.textContent = lang === "en" ? "Provider not found" : "شركة التأمين غير موجودة";
        if (shortEl) shortEl.textContent = lang === "en" ? "Please go back to Insurance page." : "ارجع إلى صفحة التأمين.";
        return;
      }
      const name = prov.name ? t(prov.name, lang) : "";
      const short = prov.short ? t(prov.short, lang) : "";
      const cov = prov.coverage ? t(prov.coverage, lang) : "";
      const logo = prov.logo != null ? String(prov.logo).trim() : "";
      const services = Array.isArray(prov.accepted_services) ? prov.accepted_services : [];

      if (nameEl) nameEl.textContent = name;
      if (shortEl) shortEl.textContent = short;
      if (logoEl) {
        logoEl.innerHTML = logo ? '<img src="' + escapeHtml(absMediaUrl(logo)) + '" alt="' + escapeHtml(name) + '" loading="lazy" decoding="async" />' : "";
      }
      if (covTitle) covTitle.textContent = lang === "en" ? "Coverage details" : "تفاصيل التغطية";
      if (covEl) covEl.textContent = cov || (lang === "en" ? "—" : "—");
      ...
      if (srvEl) {
        if (!services.length) {
          ...
          srvEl.innerHTML =
            '<div style="margin-top:1rem;font-weight:900">' +
            escapeHtml(lang === "en" ? "Accepted services" : "الخدمات المشمولة") +
            "</div>" +
  ...
  function apply(data) {
    ...
    renderNavbarCta(data, lang);
    renderFooterBranches(document.querySelector(".ps-footer"), data, lang);
    ...
    renderCareersJobs(document.getElementById("cms-careers-jobs"), data, lang);
    renderInsuranceProviders(document.getElementById("cms-insurance-providers"), data, lang);
    renderCareerJobDetail(document.getElementById("cms-career-job"), data, lang);
    renderInsuranceProviderDetail(document.getElementById("cms-insurance-provider"), data, lang);
  ...
  function safeMapSrc(s) {
    var v = s != null ? String(s).trim() : "";
    if (v.indexOf("https://www.google.com/maps/embed?") !== 0) return "";
  ...
  function buildMapsEmbedFromBranch(b) {
    if (!b || typeof b !== "object") return "";
    return safeMapSrc(b.map_embed_url || "");
  }

  function buildMapsHrefFromBranch(b, lang) {
    if (!b || typeof b !== "object") return "";
    var lat = toNum(b.map_lat);
    var lng = toNum(b.map_lng);
    if (lat != null && lng != null) {
      return "https://www.google.com/maps?q=" + encodeURIComponent(lat + "," + lng);
    }
    var q = b.map_query != null ? String(b.map_query).trim() : "";
    if (q) return "https://www.google.com/maps?q=" + encodeURIComponent(q);
    var addr = t(b.address, lang) || "";
    if (addr) return "https://www.google.com/maps?q=" + encodeURIComponent(addr);
    var raw = b.map_url != null ? String(b.map_url).trim() : "";
  ...
  var SVG_INSTAGRAM =
    ...
    '<path d="M16 11.37a4 4 0 1 1-7.914 1.173A4 4 0 0 1 16 11.37z"></path>' +
    '<line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>' +
    "</svg>";
  var SVG_FACEBOOK =
    '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
    '<path d="M22 12.06C22 6.51 17.52 2 12 2S2 6.51 2 12.06c0 5 3.66 9.15 8.44 9.94v-7.03H7.9v-2.91h2.54V9.85c0-2.51 1.49-3.9 3.77-3.9 1.09 0 2.24.2 2.24.2v2.46h-1.26c-1.24 0-1.63.77-1.63 1.56v1.87h2.78l-.44 2.91h-2.34V22c4.78-.79 8.44-4.94 8.44-9.94z"></path>' +
    "</svg>";
  var SVG_WHATSAPP =
    '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">' +
    '<path d="M20.52 3.48A11.8 11.8 0 0 0 12.06 0C5.47 0 .12 5.35.12 11.94c0 2.1.55 4.15 1.6 5.97L0 24l6.3-1.65a11.88 11.88 0 0 0 5.76 1.47h.01c6.59 0 11.94-5.35 11.94-11.94 0-3.19-1.24-6.2-3.49-8.4Zm-8.45 18.3h-.01a9.9 9.9 0 0 1-5.04-1.38l-.36-.21-3.73.98.99-3.64-.24-.37a9.89 9.89 0 0 1-1.52-5.22C2.16 6.46 6.6 2.02 12.06 2.02c2.64 0 5.11 1.03 6.98 2.89a9.83 9.83 0 0 1 2.89 6.98c0 5.46-4.44 9.89-9.86 9.89Zm5.42-7.41c-.3-.15-1.77-.87-2.04-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.95 1.17-.17.2-.35.23-.65.08-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.6.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.38-.03-.53-.08-.15-.67-1.62-.92-2.21-.24-.58-.49-.5-.67-.5h-.57c-.2 0-.53.08-.8.38-.27.3-1.05 1.02-1.05 2.49s1.08 2.9 1.23 3.1c.15.2 2.13 3.26 5.17 4.57.72.31 1.28.5 1.71.64.72.23 1.37.2 1.89.12.58-.09 1.77-.72 2.02-1.42.25-.7.25-1.3.17-1.42-.08-.12-.27-.2-.57-.35Z"></path>' +
    "</svg>";
  ...
  function whatsHrefFromPhone(phone) {
    var d = digitsOnly(phone);
    if (!d) return "";
    return "https://wa.me/" + d;
  }

  function renderNavbarCta(data, lang) {
    try {
      var phone =
        data && data.footerBranches && data.footerBranches.main && data.footerBranches.main.phone
          ? String(data.footerBranches.main.phone).trim()
          : "";
      var logoUrl = data && data.navbar && data.navbar.logo ? String(data.navbar.logo).trim() : "";
      if (logoUrl) logoUrl = absMediaUrl(logoUrl);

      document.querySelectorAll('.header-actions a.header-btn[data-translate="callNow"]').forEach(function (a) {
        if (!a) return;
        if (logoUrl) {
          a.dataset.cmsBound = "1";
          a.classList.add("header-btn--logo");
          a.removeAttribute("data-translate");
          a.setAttribute("href", "index.html");
          a.innerHTML =
            '<img class="header-logo" src="' +
            escapeHtml(logoUrl) +
            '" alt="Pearly Smile" decoding="async" loading="eager" />';
        } else {
          a.classList.remove("header-btn--logo");
          a.dataset.cmsBound = "1";
          a.setAttribute("data-translate", "callNow");
  ...
  function renderFooterBranches(footer, data, lang) {
    ...
    var fb = data && data.footerBranches;
    var branches = [];
    if (fb && typeof fb === "object") {
      Object.keys(fb).forEach(function (k) {
        var b = fb[k];
        if (b && typeof b === "object") branches.push(b);
      });
    }

    function branchCard(b) {
      if (!b || typeof b !== "object") return "";
      var name = t(b.name, lang) || "";
      var addr = t(b.address, lang) || "";
      var hours = t(b.hours, lang) || "";
      var phone = b.phone != null ? String(b.phone).trim() : "";
      var mapSrc = buildMapsEmbedFromBranch(b);
      var mapHref = buildMapsHrefFromBranch(b, lang);
      var mapHtml = mapSrc
        ? '<div class="ps-branch__map"><iframe loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="' +
          escapeHtml(mapSrc) +
          '" allowfullscreen="" aria-label="Map"></iframe></div>'
        : mapHref
          ? '<div class="ps-branch__map ps-branch__map--btn"><a class="btn btn-secondary" href="' +
            escapeHtml(mapHref) +
      ...
      var labelAddress = lang === "en" ? "Address" : "العنوان";
      var labelPhone = lang === "en" ? "Phone" : "الهاتف";
      var labelHours = lang === "en" ? "Hours" : "ساعات العمل";
      var waHref = whatsHrefFromPhone(phone);

      return (
        '<section class="ps-branch" data-cms-bound="1">' +
        '<div class="ps-branch__title">' +
        escapeHtml(name) +
        "</div>" +
        mapHtml +
        '<div class="ps-branch__meta">' +
        (addr ? "<div><strong>" + labelAddress + ":</strong> " + escapeHtml(addr) + "</div>" : "") +
        (phone
          ? '<div><strong>' +
            labelPhone +
            ':</strong> <span class="ps-phone-row"><a class="ps-phone" href="tel:' +
            escapeHtml(phone) +
            '">' +
            escapeHtml(phone) +
            "</a>" +
            ...
            "</span></div>"
          : "") +
        (hours ? "<div><strong>" + labelHours + ":</strong> " + escapeHtml(hours) + "</div>" : "") +
        "</div>" +
        "</section>"
      );
    }

    var html = branches.map(branchCard).join("");
    if (html) {
      host.innerHTML = html;
    } else {
      var emptyMsg =
        lang === "en"
          ? "Footer branches are not configured yet. Open Admin → Branches and save."
          : "لم تتم تهيئة بيانات الفروع بعد. افتح لوحة الإدارة → الفروع واحفظ.";
      host.innerHTML = '<div class="empty-hint" style="padding:1rem;opacity:.7">' + emptyMsg + "</div>";
    }

    // Enterprise meta column: links + services + contact + socials + legal
    try {
      if (metaHost) {
        var brand = lang === "en" ? "Pearly Smile Dental Center" : "مركز بيرلي سمايل لطب الأسنان";
        var tagline =
          lang === "en"
            ? "Premium dental care with comfort-first experience across our branches."
            : "رعاية أسنان متقدمة بتجربة مريحة وخطة علاج واضحة عبر فروعنا.";

        var qLinks = [
          { href: "careers.html", ar: "وظائف", en: "Careers" },
          { href: "insurance.html", ar: "التأمين", en: "Insurance" },
          { href: "faq.html", ar: "الأسئلة الشائعة", en: "FAQ" },
          { href: "contact.html", ar: "تواصل معنا", en: "Contact" },
          { href: "privacy.html", ar: "سياسة الخصوصية", en: "Privacy Policy" },
          { href: "terms.html", ar: "الشروط والأحكام", en: "Terms & Conditions" },
        ];

        var meta = (data && data.footerMeta) || {};
        var email = meta.email != null ? String(meta.email).trim() : "";
        var termsHref = meta.terms_href != null ? String(meta.terms_href).trim() : "";
        var mainPhone =
          data && data.footerBranches && data.footerBranches.main && data.footerBranches.main.phone
            ? String(data.footerBranches.main.phone).trim()
            : "";
        var waHrefMain = whatsHrefFromPhone(mainPhone);

        var socials = (data && data.footerSocials) || {};
        var ig = socials.instagram != null ? String(socials.instagram).trim() : "";
        var fbk = socials.facebook != null ? String(socials.facebook).trim() : "";
        var socHtml = "";
        if (ig) socHtml += '<a class="ps-soc" href="' + escapeHtml(ig) + '" target="_blank" rel="noopener" aria-label="Instagram">' + SVG_INSTAGRAM + "</a>";
        if (fbk) socHtml += '<a class="ps-soc" href="' + escapeHtml(fbk) + '" target="_blank" rel="noopener" aria-label="Facebook">' + SVG_FACEBOOK + "</a>";

        var qLinksHtml = qLinks
          .map(function (x) {
            var label = lang === "en" ? x.en : x.ar;
            return '<li><a href="' + escapeHtml(x.href) + '">' + escapeHtml(label) + "</a></li>";
          })
          .join("");

        // Desktop vs Mobile: render both, CSS decides which to show.
        metaHost.innerHTML =
          '<div class="ps-foot2 ps-foot2--desktop" data-cms-bound="1">' +
          '<div class="ps-foot2__grid">' +
          '<div class="ps-foot2__left">' +
          '<div class="ps-foot2__brand">' +
          escapeHtml(brand) +
          "</div>" +
          '<div class="ps-foot2__tag">' +
          escapeHtml(tagline) +
          "</div>" +
          '<div class="ps-foot2__socials">' +
          (socHtml || "") +
          "</div>" +
          "</div>" +
          '<div class="ps-foot2__right">' +
          '<div class="ps-foot2__col">' +
          '<div class="ps-foot2__title">' +
          escapeHtml(lang === "en" ? "Explore" : "استكشف") +
          "</div>" +
          '<ul class="ps-foot2__links">' +
          qLinksHtml +
          "</ul>" +
          "</div>" +
          '<div class="ps-foot2__col">' +
          '<div class="ps-foot2__title">' +
          escapeHtml(lang === "en" ? "Contact" : "التواصل") +
          "</div>" +
          '<div class="ps-foot2__contact">' +
          (email
            ? '<a href="mailto:' + escapeHtml(email) + '">' + escapeHtml(email) + "</a>"
            : "") +
          (mainPhone
            ? '<a href="tel:' + escapeHtml(mainPhone) + '">' + escapeHtml(mainPhone) + "</a>"
            : "") +
          (waHrefMain
            ? '<a href="' +
              escapeHtml(waHrefMain) +
              '" target="_blank" rel="noopener">' +
              escapeHtml(lang === "en" ? "WhatsApp" : "واتساب") +
              "</a>"
            : "") +
          "</div>" +
          "</div>" +
          ...
          "</div>" +
          "</div>" +
          '<div class="ps-foot ps-foot--mobile" data-cms-bound="1">' +
          '<div class="ps-foot__top">' +
          '<div class="ps-foot__brand">' +
          escapeHtml(brand) +
          "</div>" +
          '<div class="ps-foot__tag">' +
          escapeHtml(tagline) +
          "</div>" +
          "</div>" +
          '<div class="ps-foot__acc" role="list">' +
          '<details class="ps-foot__acc-item" open><summary>' +
          escapeHtml(lang === "en" ? "Quick Links" : "روابط سريعة") +
          '</summary><ul class="ps-foot__links">' +
          qLinksHtml +
          "</ul></details>" +
          '<details class="ps-foot__acc-item"><summary>' +
          escapeHtml(lang === "en" ? "Contact" : "التواصل") +
          '</summary><div class="ps-foot__contact">' +
          (email
            ? '<div><a href="mailto:' + escapeHtml(email) + '">' + escapeHtml(email) + "</a></div>"
            : "") +
          (mainPhone
            ? '<div><a href="tel:' + escapeHtml(mainPhone) + '">' + escapeHtml(mainPhone) + "</a></div>"
            : "") +
          (waHrefMain
            ? '<div><a href="' +
              escapeHtml(waHrefMain) +
          ...
          "</div></details>" +
          '<details class="ps-foot__acc-item"><summary>' +
          escapeHtml(lang === "en" ? "Social" : "السوشيال") +
          '</summary><div class="ps-foot__socials">' +
          (socHtml || '<span style="opacity:.7">' + escapeHtml(lang === "en" ? "—" : "—") + "</span>") +
          "</div></details>" +
          "</div>" +
          "</div>";
      }
    ...
    // Socials → SVG icons
    try {
      var bottom = footer.querySelector("[data-cms-footer-socials]");
      if (bottom) {
        var socials = (data && data.footerSocials) || {};
        var ig = socials.instagram != null ? String(socials.instagram).trim() : "";
        var fbk = socials.facebook != null ? String(socials.facebook).trim() : "";
    ...
    // Bottom legal links (right side)
    try {
      var legalHost = footer.querySelector("[data-cms-footer-legal]");
      if (legalHost) {
        legalHost.innerHTML =
          '<a href="privacy.html">' +
          escapeHtml(lang === "en" ? "Privacy Policy" : "سياسة الخصوصية") +
          "</a>" +
          ' <span style="opacity:.55">·</span> ' +
          '<a href="terms.html">' +
          escapeHtml(lang === "en" ? "Terms & Conditions" : "الشروط والأحكام") +
          "</a>";
      }
    } catch (eLeg) {
      /* no-op */
